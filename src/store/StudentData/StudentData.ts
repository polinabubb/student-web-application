...
function DeleteStudentById(id: number, students: Student[]) {
  ...
  if (index !== -1) {
    if (index === 0) {
      ...
      result = students
        .slice(0, index)
        .concat(students.slice(index + 1, students.length));
    }
  }
  return result;
}
...
export const studentData = createSlice({
  ...
  reducers: {
    ...
    sortingStudents(state, action: PayloadAction<{ sorting: SortingType }>) {
      ...
      let sortingFunction;
      if (sorting.byName) {
        sortingFunction = (a: Student, b: Student) =>
    ...
    filterStudents(state, action: PayloadAction<{ name: string }>) {
      ...
      if (name === "") {
        ...
        state.displayedStudents = state.sortingStudents.filter((student) =>
...
export const { deleteStudent, sortingStudents, filterStudents } =
  studentData.actions;
